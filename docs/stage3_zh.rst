...
每次 train 都会把同样的数据镜像到 ``<output>/tensorboard``（每个 update 的标量、
每 5 个 update 的各参数组梯度/更新范数、每次评估时各划分 correct-reference 指标均值
和前 8 张条带图），加 --no-tensorboard 可关闭。train.jsonl 和 eval 目录仍是正式记录，
TensorBoard 只是查看视图。
...
旧定义等权参数梯度范数的中位数不变。最终有效系数超出 [0.1, 100]、梯度为零或组合梯度退化都会
阻止标定通过。不要为了强行通过而放宽限制。梯度范数比值并不等于各损失对更新
的独立贡献份额。

新命令默认 ``--structure-projection-mode sum``，row/col 沿投影轴求和再对
batch 取均值；``legacy`` 保留原先沿轴取均值的定义。旧 checkpoint 缺少字段
时仍按 legacy 加载。统计区域仍是全拼接图，detail 仍使用 mask*valid。
必须重新校准并使用新输出目录，系数文件模式必须与运行模式一致。
internal 的 equal 使用 legacy 等权，calibrated 使用 sum 和新系数；这是尺度
与内部比例联合对照。若 equal 胜出，后续扫描省略系数且显式指定 legacy。
