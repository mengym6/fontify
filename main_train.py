...
def get_args_parser():
    parser = argparse.ArgumentParser('Fontify pre-training', add_help=False)
    ...
    # Model parameters
    parser.add_argument('--model', default='vit_large_patch16_input896x448_win_dec64_8glb_sl1', type=str,
                        metavar='MODEL',
...
def main(args, ds_init):
    ...
    if args.finetune:
        ...
        if "patch32" in args.model:
            ...
            new_patch_weight = torch.nn.functional.interpolate(patch_weight, size=(32, 32), mode='bicubic',
                                                               align_corners=False)
            checkpoint['model']['patch_embed.proj.weight'] = new_patch_weight
        # interpolate position embedding
        if "fontify" not in args.model:
    ...
    if global_rank == 0 and args.log_wandb:
        ...
        if args.resume == '':
            wandb.init(project="Fontify", name=experiment, config=args)
        else:
            wandb.init(project="Fontify", name=experiment, config=args, resume=True)
